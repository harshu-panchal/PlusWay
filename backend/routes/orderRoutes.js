...
const { createOrder, verifyPayment, createRazorpayOrder, verifyRazorpayPayment, createCodOrder, getMyOrders, getAllOrders, getCustomerOrders, updateOrderStatus, assignOrder } = require('../controllers/orderController');
const { protect, optionalProtect, authorize } = require('../middleware/authMiddleware');

router.post('/create', optionalProtect, createOrder);
router.post('/create-cod', optionalProtect, createCodOrder);
