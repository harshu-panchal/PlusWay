...
const DealOfTheDay = () => {
    ...
    const TimeBox = ({ value, label }) => (
        <div className="flex flex-col items-center bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-2 min-w-[50px] md:min-w-[60px]">
    ...
    return (
        <section className="my-8 mx-4 lg:mx-[10%] rounded-2xl overflow-hidden relative isolate bg-slate-900">
            {/* Background Effects */}
            ...
            <div className="flex flex-col md:flex-row gap-8 p-6 md:p-10 items-center justify-between">
                {/* Content */}
                <div className="order-2 md:order-1 text-center md:text-left flex-1 max-w-md shrink-0">
                    ...
                    <h2 className="text-2xl md:text-3xl font-bold text-white mb-3 leading-tight line-clamp-2">
                        {deal.title}
                    </h2>

                    <div className="flex items-center justify-center md:justify-start gap-3 mb-6">
                        <span className="text-3xl font-bold text-teal-400">₹{deal.dealPrice.toLocaleString()}</span>
                    ...
                    <Link
                        ...
                        className="inline-flex items-center justify-center px-6 py-3 bg-teal-500 hover:bg-teal-600 text-white font-bold rounded-lg text-sm transition-all shadow-lg hover:shadow-teal-500/30 hover:-translate-y-0.5"
                    >
                        Shop Now
                    </Link>
                </div>
                ...
                {/* Images - Row of 3 */}
                <div className="order-1 md:order-2 flex gap-4 w-full justify-center md:justify-end">
                    {deal.images.map((img, index) => (
                        <div key={index} className={`relative rounded-xl overflow-hidden bg-white/5 border border-white/10 shadow-xl transition-transform hover:scale-105 duration-300 ${index === 1 ? 'w-40 h-40 md:w-48 md:h-48 z-10 -mt-4' : 'w-28 h-28 md:w-36 md:h-36 opacity-80 hover:opacity-100'}`}>
                            <img
                                src={img}
                                alt={`View ${index + 1}`}
